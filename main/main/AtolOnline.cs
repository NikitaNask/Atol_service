    ...
    public class POSAtolOnline
    {
        ...
        public string SendPaymentATOL(ServiceConfig m_Config, kkmServiceRequest m_request, CultureInfo FormatProvider, out string UUID, out string exeptionMessage, out string status)
        {
            ...
            try
            {
               ...
               // m_request.groupOperId = 319979252;
                ...
                m_request.email = "receipt@test.ru";

                string pData = @"{ ""external_id"":""" + m_request.groupOperId.ToString() + @""", 
                                        ""receipt"":
                                        {""client"":{""email"":""" + m_request.email + @"""},
                                        ""company"":
                                        {""email"":""" + m_Config.emailCompany + @""",
                                        ""inn"":""" + m_Config.inn + @""",
                                        ""payment_address"":""" + m_Config.paymentAddr + @"""},
                                        ""items"":
                                        [{""name"":""" + m_Config.nameServices + m_request.accountNumber + @""",
                                        ""price"":" + summ + @", 
                                        ""quantity"":1,
                                        ""sum"":" + summ + @",
                                        ...
                                        ""total"":" + summ + @"},
                                        ""service"":
                                        {""callbackUrl"":""" + m_Config.callBackUrl + @"""}, 
                                        ""timestamp"":""" + timestamp + @"""}";
                string jSonStr = pData;
                
                // Serializer.ToJson<AtoRequest>(atolRequest);
